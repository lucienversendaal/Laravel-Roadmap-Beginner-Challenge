                    ...
                    <form action="{{ route('admin.articles.update', $article->id) }}" method="POST"
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="content" id="content" rows="5"
                                class="w-full px-3 py-2 leading-tight text-gray-700 border rounded shadow appearance-none focus:outline-none focus:shadow-outline">{{ $article->content }}</textarea>
                            @error('content')
                                <p class="text-xs italic text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select type="text" name="category" id="category"
                                ...
                                @foreach ($categories as $key => $category)
                                    <option value="{{ $article->category_id }}">{{ $category->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-4">
                            ...
                            @error('image')
                                <p class="text-xs italic text-red-500">{{ $message }}</p>
                            @enderror
                            <img id="preview" src="{{ $article->getImage() }}" alt="your image"
